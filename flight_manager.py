...
class FlightEditor(Toplevel):
    def __init__(self, app: "FlightManagerApp", record: dict[str, str] | None = None, focus_field: str | None = None):
        ...
        button_bar.grid(row=len(REQUIRED_FIELDS) + 1, column=0, columnspan=3, sticky="e", pady=(14, 0))
...
class FlightManagerApp:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(search_frame, text="删除选中", command=self.delete_selected).grid(row=0, column=12)
        ...
        self.reminder_note = ttk.Label(reminder_frame, text="单击任意提醒项即可打开对应航线补录。", foreground="#B91C1C", wraplength=320)
        ...
        self.reminders.heading("missing", text="缺失字段")
    ...
    def refresh(self, select_id: str | None = None) -> None:
        ...
        for record in self.current_results:
            ...
            tag = "missing" if missing else "complete"
        ...
        self.status_var.set(f"共 {len(self.records)} 条记录，当前显示 {len(self.current_results)} 条，待补录 {missing_count} 条")
    ...
    def delete_selected(self) -> None:
        ...
        if not record:
            return
        if not messagebox.askyesno("确认删除", f"确定删除这条记录吗？\n\n{record_summary(record)}", default=messagebox.NO, parent=self.root):
            return
        self.records = [item for item in self.records if item.get("id") != record.get("id")]
    ...
    def open_reminder_from_click(self, event) -> None:
        ...
        if not record:
            return
        first_missing = missing_fields(record)[0] if missing_fields(record) else None
        self.edit_record_by_id(row_id, focus_field=first_missing)
